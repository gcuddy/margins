...
export const GET: RequestHandler = async (e) => {
	console.log("RECEIVED REFRESH TOKEN REQUEST", e.url.searchParams.get("refresh_token"));
	const refresh_token = e.url.searchParams.get("refresh_token");
	if (!refresh_token) throw error(400, "No refresh token provided");
	...
	const res = await e.fetch(api, {
		...
		headers: {
			...
			Authorization:
				"Basic " + Buffer.from(PUBLIC_SPOTIFY_CLIENT_ID + ":" + SPOTIFY_CLIENT_SECRET).toString("base64"),
		},
		body: form,
	});
	console.log({ res });
