...
async function processFeed({ feedUrl, id: feedId, lastParsed }: z.infer<typeof feedSchema>) {
    try {
        if (!feedUrl) return;
        ...
        const newItems = feed.items.filter((item) => {
            return true;
            const pub_date = item.isoDate ?? item.pubDate ?? item.published ?? item.updated;
            if (!pub_date) {
        ...
        const entries = await Promise.all(newItems.map(item => adaptEntryFromItem(item, feedId)));
