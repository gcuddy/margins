					...
					<div class="space-y-2">
						{#each conditions as condition, index (condition.id)}
							<div class="flex space-x-1">
								<Select class="h-7 w-auto flex-initial py-1  text-xs" bind:value={condition.field}>
									<option value="author">Author</option>
									<option value="title">Title</option>
									<option value="url">URL</option>
								</Select>
								<Select
									class="h-7 w-auto flex-initial py-1 text-xs"
									name="{condition.field}-filter"
									bind:value={condition.filter}
								>
