...
export const load = (async (event) => {
	...
	switch (type) {
		...
		case 'podcast': {
			...
			if (podcastIndexId) {
				query = query.where('Entry.podcastIndexId', '=', BigInt(podcastIndexId));
	...
	const updateBookmarkForm = superValidate<typeof updateBookmarkSchema, Message>(
		entry?.bookmark,
	...
	return {
		...
		extras: {
			season:
				type === 'tv' && event.url.searchParams.get('tab') === 'episodes'
					? tmdb.tv.season(+id, +(event.url.searchParams.get('season') ?? '1'))
					: null
		}
	};
