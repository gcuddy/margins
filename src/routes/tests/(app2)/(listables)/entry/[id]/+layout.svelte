...
<div class="relative flex items-start gap-10">
	...
	<aside class="sticky top-10 hidden w-72 space-y-4 xl:block">
		<!--  -->
		<H3>Metadata</H3>
		<div class="flex items-center space-x-4">
			<Subtle>Tags</Subtle>
			<!-- <H4>Tags</H4> -->
			<TagPopover data={data.tagForm} entry={$page.data.entry} />
		</div>
		<div class="flex items-center space-x-4">
			<Subtle>Status</Subtle>
			<!-- <H4>Tags</H4> -->
			<StatusPopover data={data.updateBookmarkForm} entry={$page.data.entry} />
		</div>
		{#key data.entry.id}
			<TableOfContents
				active="font-bold"
