...
import { fetchRss, inputSchema as rssInputSchema } from "./(listables)/subscriptions/latest/fetch.server";
...
interface Query<I extends (z.ZodTypeAny), Data> {
    staleTime?: number;
    fn: (args: {
        input: I extends z.ZodType<object> ? z.infer<I> : undefined, ctx: {
            userId: string;
        }
    }) => Promise<Data>;
    schema?: I;
...
export const mutations = ({
    ...
    updateBookmark: query({
        schema: updateBookmarkSchema.and(z.object({
            userId: z.string().optional()
        })),
        fn: async ({ input, ctx }) => updateBookmark({
            ...input,
            userId: ctx.userId
        })
    }),
    createCollection: query({
        ...
        fn: async ({ ctx, input }) => {
            const result = await db.insertInto("Collection")
                .values({
                    name: input.name,
                    userId: ctx.userId,
                    updatedAt: new Date()
    ...
    addRelation: query({
        ...
        fn: async ({ input, ctx }) => {
            return await db.insertInto("Relation")
                .values({
    ...
    deleteInteraction: query({
        ...
        fn: async ({ input, ctx }) => {
            await db.deleteFrom("EntryInteraction")
                .where("id", "=", input.id)
...
export const queries = ({
    ...
    findOrCreateEntry: query({
        ...
        fn: async ({ input }) => {
            if (input.tmdbId) {
                ...
                const new_entry = await db.insertInto("Entry")
                    .values({
                        title: movie.title,
                        html: movie.overview,
                        uri: `tmdb:${movie.id}`,
    ...
    entries_by_tag: query({
        ...
        fn: async ({ input, ctx }) => {
            let query = db.selectFrom("TagOnEntry as toe")
                ...
                .innerJoin("Tag as t", (join) => join.onRef("t.id", "=", "toe.tagId").on("t.name", "=", input.name))
                .select(entrySelect)
    ...
    collections: query({
        staleTime: 1000,
        fn: ({ ctx }) => {
            return db.selectFrom("Collection")
                .where("userId", "=", ctx.userId)
                .selectAll()
                .execute();
        },
    ...
    note_mentions: query({
        ...
        fn: async ({ ctx, input: { title, id } }) => {
            const references = db.selectFrom("Annotation")
                .select(eb => jsonArrayFrom(eb.selectFrom("annotation_to_entry_reference as r")
                ...
                .select(eb => jsonArrayFrom(eb.selectFrom("Annotation as a")
