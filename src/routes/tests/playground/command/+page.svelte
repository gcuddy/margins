<script lang="ts">
	...
	const {
		...
		filterFunction,
		items: [...books, ...books, ...books, ...books, ...books, ...books, ...books, ...books, ...books, ...books],
...
<div class="flex flex-col gap-1">
	...
	<div class="relative">
		<input
            ...
            px-3 pr-12 text-stone-700"
			placeholder="Best book ever"
		/>
		<div class="absolute right-1 top-1/2 z-10 -translate-y-1/2 text-stone-700">
			{#if $open}
			...
			{#each $filteredItems as book, index (index)}
				<li
					...
					class="relative cursor-pointer rounded-md py-1 pl-8 pr-4 text-neutral-800
                          data-[highlighted]:bg-red-100 data-[highlighted]:text-stone-700
