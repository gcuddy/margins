...
{#each types as type}
	...
	{#each $page.data.user?.states?.filter((state) => state.type === type.toLowerCase()) || [] as state}
		{@const defaultState = state.id === $page.data.user?.default_state_id}
		{@const editing = editing_id && +editing_id === state.id}
		{#if editing}
			<form
				class="group flex items-center rounded bg-gray-100 p-2 shadow {loading ? 'animate-pulse' : ''}"
				method="post"
				action="?/update"
				use:enhance={() => {
					loading = true;
			...
			>
				...
				>
				<Button disabled={loading} type="submit" size="sm" variant="naked"><Muted>Save</Muted></Button>
			</form>
		{:else}
			<div
			...
			>
				...
				<div class="flex grow items-center gap-2">
					<Icon name={LOCATION_TO_ICON_SOLID[state.type]} className="h-4 w-4 fill-[var(--state-color)]" />
				...
				{#if !defaultState}
					<form action="?/delete" method="post" use:enhance>
						<input type="hidden" name="id" value={state.id} />
						<Button type="submit" className="group-hover:opacity-100 opacity-0" size="sm" variant="naked"
							><Muted>Delete</Muted></Button
						>
					</form>
				{/if}
				{#if defaultState}
