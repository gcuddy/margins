		...
		>
			...
			{#if $podcast}
				<div
					...
					class="max-h-60 w-60 place-self-center overflow-hidden rounded-xl shadow-lg ring-1 ring-border/50 dark:shadow-2xl dark:shadow-[var(--shadow-color)] sm:place-self-start"
				>
				...
				{:else if $podcast.isSuccess && $podcast.data}
					<div class="space-y-4 sm:space-y-8">
						<div class="flex flex-col gap-1 text-center sm:text-left">
							...
							<a class="text-xl" href={$podcast.data.url}
								><Muted>{$podcast.data.author}</Muted></a
							>
						...
						<div class="relative overflow-hidden text-sm line-clamp-4">
							...
							<GenericDialog bind:isOpen={show_description_modal}>
								...
								<span class="font-bold" slot="description"
									>{$podcast.data.title}</span
								>
						...
						<form method="post" action="?/subscribe" use:enhance={handleSubmit}>
							...
							<input
								type="hidden"
								name="podcastIndexId"
								value={$podcast.data.id}
							/>
							<Button
								type="submit"
								className="flex space-x-2 text-lg py-4 px-3"
							>
			...
			<div class="flex flex-col dark:divide-gray-700">
				<ol class="divide-y divide-border dark:divide-gray-700">
					{#if $episodes.isLoading}
						{#each Array(10) as _}
							<div
								class="animate-pulsing flex items-center justify-between py-4 transition-opacity"
							>
								<div>
									<div
					...
					{:else if $episodes.isSuccess && $episodes.data}
						{#each $episodes.data.episodes.items.slice(0, show) as item}
							{@const loaded =
								$podcastPlayer?.episode?.enclosureUrl === item.enclosureUrl}
							{@const entry = $episodes.data.entries.find(
								(e) =>
									e.podcastIndexId?.toString() == item.id.toString() ||
									e.enclosureUrl == item.enclosureUrl
							)}
							...
							<li class="py-2">
								<div class="flex flex-col space-y-1">
									...
									<div
										class="max-h-20 overflow-hidden text-sm text-muted line-clamp-3"
									>
									...
									<div class="flex justify-between gap-2">
										...
										>
											<div class="flex flex-col items-center">
												<Icon
													name={loaded && !$podcastPlayer.paused
														? "pauseSolid"
														: "playSolid"}
											...
											>
												...
												<Muted
													>{loaded
														...
														? formatDuration(
																item.duration -
																	item.duration * interaction.progress,
																"seconds"
														  ) + " left"
												...
												use:enhance={() => {
													...
													return ({ result }) => {
														data.queryClient.invalidateQueries(
															podcastEpisodesQuery($page, data.id).queryKey
														);
											...
											>
												<button>
													...
													<input
														type="hidden"
														name="enclosureUrl"
														value={item.enclosureUrl}
													/>
													<input
														type="hidden"
														name="finished"
														value={!interaction?.finished}
													/>
													<Icon
														name={pending_favorite === item.id
															? "loading"
															: "checkCircle2"}
														className="h-6 w-6 stroke-gray-500 transition hover:stroke-white {interaction?.finished
															? 'opacity-100'
															: 'opacity-50'} {pending_favorite === item.id
															? 'animate-spin'
															: ''}"
													/>
												</button>
