...
<Header>
	<DefaultHeader>
		...
		<div slot="end">
            <a href="/u:{data.user?.username}/smart/playground" class="flex items-center space-x-2"> playground </a>
			<Button as="a" href="/u:{data.user?.username}/smart/new" variant="ghost" className="flex"
	...
	<div class="flex flex-col">
		{#each lists as list, index}
			...
			<KeyboardNavItem
				...
				href="/u:{data.user?.username}/smart/{list.id}"
				class={({ active }) =>
			...
			>
				<div class="flex h-full  items-center space-x-4 border-b py-3 px-6 dark:border-gray-700 lg:px-9">
