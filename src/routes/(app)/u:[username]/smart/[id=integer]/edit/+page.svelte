<script lang="ts">
	...
	const mutation = createMutation({
        ...
        onSuccess: (data) => {
            ...
            if (data) goto(`/u:${$page.data.user?.username}/smart/${data.id}`)
        }
	});
</script>

<EntryFilter let:query let:where let:chosenConditions {conditions} {and}>
			...
			<div class="flex flex-col space-y-2 divide-y p-4 dark:divide-gray-700">
				...
				<div class="flex flex-col gap-4 pt-4 md:flex-row md:justify-between">
					...
					<div class="flex space-x-2">
						<Button variant="link" as="a" href="/u:{$page.data.user?.username}/smart/{data.list?.id}">Cancel</Button>
						<Button
							on:click={async () => {
