...
<div class="container mx-auto px-4">
	...
	{#if !form || form?.message}
		{#if form?.message}
			<p>{form.message}</p>
		{/if}
		<form use:enhance action="?/submitOpml">
			<label class="block"
		...
		>
			<div class="flex flex-col space-y-4 border">
				...
				<ul class="max-h-52 overflow-y-auto">
					{#each form.opml.body.outline.filter((o) => o.type === 'rss' && o.xmlUrl) as item}
						<li class="flex items-center space-x-4 px-2">
							<input type="checkbox" name="url" value={item.xmlUrl} class="shrink-0" />
