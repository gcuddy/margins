...
export type QueryInput<TKey extends keyof Queries> = Parameters<Queries[TKey]["fn"]>[0]["input"] extends IsAny<Parameters<Queries[TKey]["fn"]>[0]["input"]> ? undefined : Parameters<Queries[TKey]["fn"]>[0]["input"];
export type QueryOutput<TKey extends keyof Queries> = Awaited<ReturnType<Queries[TKey]["fn"]>>;
export type MutationInput<TKey extends keyof Mutations> = Parameters<Mutations[TKey]["fn"]>[0]["input"] extends IsAny<Parameters<Mutations[TKey]["fn"]>[0]["input"]> ? undefined : Parameters<Mutations[TKey]["fn"]>[0]["input"];
...
export type InfiniteQueries = {
    [K in keyof Queries]: Parameters<Queries[K]["fn"]>[0]["input"] extends { cursor: unknown } ? K : never;
}[keyof Queries];
...
export async function query<T extends keyof Queries>(
    ...
    userId = userId || init.data?.userId || init.data?.session?.userId || init.data?.user?.userId || null;
...
export async function mutation<T extends keyof Mutations>(
    ...
    fn: T,
    input: Parameters<Mutations[T]["fn"]>[0]["input"] extends IsAny<Parameters<Mutations[T]["fn"]>[0]["input"]> ? undefined : Parameters<Mutations[T]["fn"]>[0]["input"],
    opts?: Partial<{
        fetcher: typeof fetch;
        userId: string | null;
    }>
