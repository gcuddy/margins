...
export function parse_annotations(pdf: PDFDocumentProxy) {
...
export async function get_pdf_fingerprint(pdf: PDFDocumentProxy) {
	return pdf.fingerprints[0];
}

export async function parse_pdf(pdf: PDFDocumentProxy, parseText = false) {
...
export async function get_pdf_text(
	...
	return opts.type === 'html'
		? pages.then(pages_to_html)
		: pages.then(pages_to_string);
}
...
const pages_to_string = (pages: Array<Page>) =>
	pages.reduce((accum, page) => accum.concat(`${page.lines.join('\n')}\n`), '');

const pages_to_html = (pages: Array<Page>) =>
	pages.reduce(
		(accum, page, index) =>
			accum.concat(
				`<div class="page" data-page=${index + 1}>${page.lines.join(
					'<br/>',
				)}</div>`,
			),
		'',
...
async function read_pdf_text(
	...
	for (let i = 1; i <= num_pages; i++) {
		pages.push(await parse_page(await pdf.getPage(i)));
	}

	return pages;
}

async function parse_page(page: PDFPageProxy): Promise<Page> {
 ...
 */
const parse_page_items = (pdfItems: Array<TextItem>): Page => {
	const lineData: { [y: number]: Array<TextItem> } = {};
	...
	const yCoords = Object.keys(lineData)
		...
		.reduce((accum: Array<number>, currentY, index, array) => {
