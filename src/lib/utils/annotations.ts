...
export function generateTextFragmentLink(
	baseUrl: string,
	annotation: {
		exact: string;
		prefix?: string;
	...
	if (typeof prefix === 'string' && typeof suffix === 'string') {
		return `${baseUrl}#:~:text=${encodeText(prefix)}-,${encodeText(
			exact.split(' ').slice(0, 5).join(' '),
		)},-${encodeText(suffix)}`;
	}
}

export function getTargetSelector<TType extends Selector['type']>(
	_target: unknown | TargetSchema,
	type: TType,
...
export async function triggerDownload(
	{ title, author, uri }: Pick<Entry, 'title' | 'author' | 'uri'>,
	...
	for (const annotation of annotations) {
		const quote = getTargetSelector(
			annotation.target as TargetSchema,
			'TextQuoteSelector',
		);
		const body =
			annotation.body ??
			((await get(`note_markdown:${annotation.id}`)) as string | undefined);
		let text = '';
		if (quote) {
			text = `> ${quote.exact}\n`;
		}
