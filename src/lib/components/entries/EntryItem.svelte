<script lang="ts">
	...
	import { Check, Play } from 'radix-icons-svelte';
	...
	import { cmdOrCtrl, isBrowser } from '$lib/helpers';
	...
	import { getId, getType, make_link, make_url } from '$lib/utils/entries';
	...
	function copy() {
		...
		if (url) {
			...
			copied = true;
            // From a design perspective would prefer to not trigger toast here, and have the notification/feedback be inline.
            // See https://interfaces.rauno.me/
            toast.info("Copied URL", {
                description: "Paste it anywhere to share this entry",
                duration: 1500
            })
			setTimeout(() => {
				copied = false;
...
<a
	...
	on:keydown={(e) => {
		// TODO: maybe this should be hoisted to parent component to not create a bunch of listeners?
		// listen for copy event - oncopy doesn't seem to work though
		if (e.key === 'c' && cmdOrCtrl(e)) {
			e.preventDefault();
			e.stopPropagation();
			copy();
		}
...
>
	<div
		class={cn(
			...
			'bg-[inherit] group-data-[active=true]/container:ring-[0.5px] ring-inset relative',
	...
	>
		{#if view === 'list'}
			{#if viewPreferences.image}
				<Tooltip.Root openDelay={checked ? 1000 : 300}>
					<Tooltip.Trigger asChild let:builder>
						<div
							use:melt={builder}
							class="group/select relative h-8 w-8 sm:h-10 sm:w-10 shrink-0 rounded-md object-cover ring-offset-background group-focus-within:ring-2 group-focus-within:ring-ring group-focus-within:ring-offset-2"
						>
							{#if copied}
								<div
									transition:fade={{
									...
									class="absolute rounded inset-0 z-10 bg-background flex flex-col items-center justify-center w-full h-full"
								>
									<Check />
								</div>
							{/if}
			...
			{/if}
			<div class="flex min-w-0 gap-x-4 relative grow">
