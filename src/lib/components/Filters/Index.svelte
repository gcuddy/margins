<script lang="ts" context="module">
	...
	export const Filter = z.union([BooleanFilter, StringFilter, DateFilter]);
	...
	export function buildFilter(
		filterOption: ChildFilterOption
	): (item: Input, value?: string) => boolean {
...
>
	<div let:active class="flex h-8 items-center gap-2 px-2 text-sm">
		...
		{/if}
		<span class="grow {active ? 'text-gray-100' : 'text-gray-400'}"
			>{value.display || value.name}</span
		>
	</div>
</GenericCombobox>
