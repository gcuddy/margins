...
{#if $podcastPlayer?.episode?.enclosure}
	<figure
		...
		class="fixed bottom-0 right-0 w-full bg-white/90 p-1 shadow-2xl ring-1 ring-black/5 backdrop-blur-md dark:bg-black sm:bottom-4 sm:right-4 sm:w-80 sm:rounded-lg sm:p-3"
	>
		<div class="w-full px-2 py-1">
			<div class="flex items-center justify-between sm:flex-col sm:space-y-3">
				<div
					class="flex w-full cursor-default items-center gap-4"
					on:click={(e) => goto(`/rss/podcasts/+${$podcastPlayer.episode.id}`)}
				>
					<img
						src={$podcastPlayer.podcast?.image}
						class="h-12 w-12 rounded-lg shadow sm:h-14 sm:w-14"
						alt="Artwork for {$podcastPlayer.podcast.title}"
					/>
					<div class="flex grow flex-col space-y-1 truncate text-sm sm:space-y-0">
						<span class="font-medium">{$podcastPlayer.episode.title}</span>
						<Progress
				...
				<div class="flex items-center justify-between sm:w-full">
