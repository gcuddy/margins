...
<div use:portal>
	{#if $open}
		<div in:fade={{duration: 150}} melt={$overlay} class="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm" />
		<div
			class={cn(
				...
				<div class="flex flex-col space-y-1.5 text-center sm:text-left">
					<slot name="header" title={$title} description={$description}>
						{#if $$slots.title || internal_title}
							<h2 melt={$title} class="text-lg font-semibold leading-none tracking-tight">
								<slot name="title">
									{internal_title}
								</slot>
							</h2>
						...
						{#if $$slots.description || internal_description}
							<div melt={$description} class="text-sm text-muted-foreground">
								<!--  -->
