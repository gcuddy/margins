...
{#if items.length && $virtualizer}
	<!-- todo: virtual list -->
	<KeyboardNav
		bind:container={ref}
		class=" h-56 overflow-auto outline-2 outline-red-500 will-change-transform {viewOptions.view === 'grid'
			? 'TODO:customized-color-here'
			: ''} "
	>
		<div
			class="mx-auto {viewOptions.view === 'grid'
				? 'p-4'
				: ''} relative w-full space-y-0 {viewOptions.view === 'grid'
		...
		>
			{#each $virtualizer.getVirtualItems() || [] as row, index (row.key)}
										...
										>
											<span
												class="cursor-pointer {font === 'newsreader'
													...
													: ''}"
											>
												{#if html}{@html data.title || "[No title]"}{:else}{data?.title || "[No title]"}{/if}
											</span>
