	...
	if (amazonBooksRegex.test(url)) {
		...
		if (htmlToParse) {
			...
			if (rawText) {
				...
				const isbnIndex = chunks.findIndex((t) =>
					t.toLowerCase().includes('isbn-13'),
				);
	...
	if (
		url.includes(
			'books.google.com/books/about' ||
				url.includes('google.com/books/edition'),
		)
	) {
		...
		if (id) {
			...
			return {
				...
				duration: item?.contentDetails?.duration
					? dayjs.duration(item?.contentDetails?.duration).asSeconds()
					: undefined,
				// extended: {
