...
export function get_state_context() {
	...
	if (!ctx) {
		throw new Error(
			'No state context found. Did you forget to wrap your app in a <State /> component?',
		);
	}
	return ctx;
}
...
function async_derived<T, R>(
	store: Readable<T>,
	callback: ($store: T) => Promise<R>,
	initial_value?: R,
) {
	let previous = 0;
...
export function create_state<TKey extends keyof Queries>(
	init: QueryInit,
	key: TKey,
	input: Parameters<Queries[TKey]['fn']>[0]['input'],
	opts?: Opts,
): QueryState<Awaited<ReturnType<Queries[TKey]['fn']>>>;
...
export function create_state<TKey extends keyof Queries>(
	...
	type Store = IndividualState<Return>;
	...
	const should_fetch = (state: Pick<Store, 'data' | 'updated_at'>) => {
		if (
			state.data === null ||
			Date.now() - state.updated_at > (opts?.staleTime || 0)
		) {
			return true;
		}
		return false;
	...
	const data_derived = derived<typeof options, Store>(
		options,
		($options, set) => {
			const start = Date.now();
			...
			if (
				...
				query(init || get(page), $options.key, $options.input)
					.then((value) => {
						console.log({ value });
