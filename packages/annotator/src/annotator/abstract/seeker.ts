...
export class TextSeeker<TChunk extends Chunk<string>>
		...
		if (this.position <= target) {
			...
			while (true) {
				...
				if (endOfChunk <= target) {
					...
					if (read)
						result += this.currentChunk.data.slice(
							this.offsetInChunk,
							newOffset,
						);
			...
			while (this.position > target) {
				if (this.currentChunkPosition <= target) {
					...
					if (read)
						result =
							this.currentChunk.data.slice(newOffset, this.offsetInChunk) +
							result;
					this.offsetInChunk = newOffset;
					break;
				} else {
	...
	_readToNextChunk(): [string, TChunk | null] {
		const data = this.currentChunk.data.slice(Math.max(0, this.offsetInChunk));
	...
	_readToPreviousChunk(): [string, TChunk | null] {
		const data = this.currentChunk.data.slice(
			0,
			Math.max(0, this.offsetInChunk),
		);
