...
import { DocumentType } from '@margins/db/kysely/enums';
...
function isProbablyVisible(node: HTMLElement) {
	return (
		...
		(!node.hasAttribute('aria-hidden') ||
			...
			node.classNames.indexOf('fallback-image') !== -1)
	);
}
...
const matchString = (test: string | Array<string>, domain: string): boolean => {
...
export class Parser {
	...
	async parse() {
		...
		// 		this.metadata.html =
		// 			root?.outerHTML || article.content || this.metadata.html;
		...
		// 		const appleTouchIcon = this.root.querySelector(
	...
	private getMetadata() {
		...
		for (const metaKey of Object.keys(
			metadataToExtractorHash,
		) as Array<MetadataToExtractorKeys>) {
					...
					extractor.forEach((e) => {
						...
						if (typeof e === 'string' || Array.isArray(e)) {
							...
							this.metadata[metaKey] =
								this.extractFromSelectors(this.root, [e]) ?? '';
						} else if (typeof e === 'function') {
							this.metadata[metaKey] = e(this.root);
						} else {
							// then we're looking for a meta tag
							this.metadata[metaKey] =
								this.scrapeMetaTags(metaEls, e.meta || []) ?? '';
						}
						if (this.metadata[metaKey]) return;
					});
				});
	...
	private cleanMetadata() {
		...
		if (this.metadata.author) {
			...
			if (
				...
				this.metadata.author.toLowerCase().startsWith('by ')
			) {
	...
	private scrapeMetaTags(metaEls: Array<HTMLElement>, names: Array<string>) {
	...
	private _grabArticle(opts?: {
		...
		let { stripUnlikelyCandidates = true, shouldRemoveTitleHeader = true } =
			opts || {};
		...
		const elementsToScore: Array<HTMLElement> = [];
		while (node) {
			...
			if (stripUnlikelyCandidates) {
				...
				if (role && UNLIKELY_ROLES.includes(role)) {
					console.log(
						'Removing content with role ' + role + ' - ' + matchString,
					);
					node = removeAndGetNext(node);
					continue;
				}
			...
			if (node.tagName === 'DIV') {
				...
				while (childNode) {
					...
					if (this.isPhrasingContent(childNode)) {
						...
						while (
							(p as HTMLElement).lastChild &&
							this.isWhitespace((p as HTMLElement).lastChild as HTMLElement)
						) {
							// (p as HTMLElement).lastChild.remove();
							(p as HTMLElement).removeChild(
								(p as HTMLElement).lastChild as HTMLElement,
							);
						}
						p = null;
					}
		...
		const topCandidates: Array<HTMLElement> = [];
		for (const candidate of candidates) {
			// Scale the final candidates score based on link density. Good content should have a
			// relatively small link density (5% or less) and be mostly unaffected by this operation.
			this.updateScore(
				candidate,
				(contentScore) => contentScore * (1 - this.getLinkDensity(candidate)),
			);
			...
			for (let t = 0; t < nbTopCandidates; t++) {
				...
				if (
					!aTopCandidate ||
					candidateScore > (this.nodeScoreMap.get(aTopCandidate) || 0)
				) {
		...
		if (topCandidate === null) {
			...
			const alternativeCandidateAncestors: Array<Array<HTMLElement>> = [];
			...
			while (
				...
				parentOfTopCandidate.childNodes.length == 1
			) {
				topCandidate = parentOfTopCandidate;
				parentOfTopCandidate = topCandidate.parentNode;
			}
		...
		for (const sib of siblings) {
			...
			console.log(
				...
				sibling,
				this.nodeScoreMap.has(sibling)
					? 'with score ' + this.nodeScoreMap.get(sibling)
					: '',
			);
			let append = false;
			if (sibling === topCandidate) {
				...
				if (
					sibling.classNames === topCandidate.classNames &&
					topCandidate.classNames !== ''
				)
					contentBonus += topCandidateScore * 0.2;

				if (
					this.nodeScoreMap.has(sibling) &&
					(this.nodeScoreMap.get(sibling) || 0) + contentBonus >=
			...
			if (append) {
				...
				siblings = parentOfTopCandidate.childNodes.filter(
		...
		if (textLength < 500) {
			...
			console.warn(
				'Article content length less than 500 characters. Re-running grabArticle with different flags.',
			);
	...
	private prepArticle(el: HTMLElement) {
		...
		for (const topCandidate of el.childNodes) {
			this.cleanMatchedNodes(
				topCandidate as HTMLElement,
				(node, matchString) => {
					return (
	...
	private cleanMatchedNodes(
		el: HTMLElement,
		fn: (el: HTMLElement, matchString: string) => boolean,
	) {
	...
	private clean(el: HTMLElement, tag: string) {
		...
		const tagsToRemove = el.getElementsByTagName(tag).filter(() => {
	...
	cleanStyles(e: HTMLElement) {
		...
		for (let i = 0; i < PRESENTATIONAL_ATTRIBUTES.length; i++) {
			const pres = PRESENTATIONAL_ATTRIBUTES[i];
			if (pres) e.removeAttribute(pres);
		}
		...
		const childNodes = e.childNodes.filter(
			(node) => node.nodeType === 1,
	...
	private cleanConditionally(e: HTMLElement, tag: string) {
		...
		const tagsToRemove = tagsList.filter((node) => {
			...
			if (!isList) {
				...
				listNodes.forEach(
					(listNode) => (listLength += this.getInnerText(listNode).length),
				);
			...
			if (this.getCharCount(node, ',') < 10) {
				...
				const headingDensity = this.getTextDensity(node, [
					'h1',
					'h2',
					'h3',
					'h4',
				...
				for (const embed of embeds) {
					...
					if (
						...
						REGEXPS.videos.test(embed.innerHTML)
					) {
						return false;
					}
					embedCount++;
				...
				const haveToRemove =
					...
					(!isList &&
						...
						contentLength < 25 &&
						(img === 0 || img > 2) &&
						!this.hasAncestorTag(node, 'figure')) ||
	...
	private hasChildBlockElement(element: HTMLElement): boolean {
		return element.childNodes.some((node) => {
			return (
				DIV_TO_P_ELEMS.has((node as HTMLElement).tagName) ||
				this.hasChildBlockElement(node as HTMLElement)
			);
		});
	}

	private getTextDensity(e: HTMLElement, tags: Array<string>): number {
		const textLength = this.getInnerText(e, true).length;
		if (textLength === 0) return 0;
		let childrenLength = 0;
		const children = e.querySelectorAll(tags.join(','));
		children.forEach(
			(child) => (childrenLength += this.getInnerText(child, true).length),
		);
	...
	private hasSingleTagInsideElement(node: HTMLElement, tagName: string) {
		...
		if (
			...
			(node.childNodes[0] instanceof HTMLElement &&
				node.childNodes[0]?.tagName !== tagName)
		) {
			return false;
		}
	...
	private isPhrasingContent(node: HTMLElement): boolean {
		return (
			...
			((node.tagName === 'A' ||
				node.tagName === 'DEL' ||
				node.tagName === 'INS') &&
				node.childNodes.every((node) =>
					this.isPhrasingContent(node as HTMLElement),
				))
		);
	}
	...
	private headerDuplicatesTitle(node: HTMLElement) {
		...
		console.log(
			'Evaluating simliarity of header',
			heading,
			this.metadata.title,
		);
	...
	private scrapeJsonLd() {
		const jsonLd = this.root?.querySelectorAll(
		...
		const articleJson = json.filter((json: any) =>
			new RegExp(['Article', 'Blog', 'NewsArticle'].join('|')).test(
				json['@type'],
			),
		);
		console.log({ articleJson });
		this.metadata.title = this.getJsonLdvalue(
			articleJson,
			'title',
			'name',
			'headline',
		);
		this.metadata.image = this.getJsonLdvalue(articleJson, [
			'image',
			'url',
			(a) => a[0].url,
		]);
		...
		this.metadata.siteName = this.getJsonLdvalue(
			articleJson,
			'publisher',
			'name',
		);
		...
		const recipeJson = json.find((json: any) => {
			console.log({ json });
	 ...
	 */
	private getJsonLdvalue(
		jsons: Array<any>,
		...keys: Array<
			string | [string, string] | [string, string, (o: Array<any>) => any]
		>
	): string {
		...
		for (const json of jsons) {
			for (const key of keys) {
				if (typeof key === 'string') {
					...
					console.log(
						`Searching for ${key[0]} & ${key[1]} in ${JSON.stringify(json)}`,
					);
					const firstValue = clarifyStringOrObject(json[key[0]]);
					if (Array.isArray(firstValue)) {
						console.log(
							`Searching for ${key[1]} in ${JSON.stringify(firstValue)}`,
						);
						const arr = firstValue;
						const secondaryKey = key[1];
						console.log({ arr, secondaryKey });
						const fn =
							key[2] || ((o) => o.map((o) => o[secondaryKey]).join(', '));
						console.log({ fn });
						const filtered = arr.filter((a) =>
							a && typeof a === 'object' && secondaryKey in a
								? a[secondaryKey as keyof typeof a]
								: a,
						);
			...
			if (element && typeof selector === 'string') {
				...
				try {
					if (text) {
						...
						if (json && json[selector[2] as keyof typeof json]) {
							// console.log(`and here's the text:`, json[selector[2]]);
							return json[selector[2] as keyof typeof json];
						}
					}
				} catch (error) {
	...
	private fixLazyImages(node: HTMLElement) {
		...
		for (const elem of els) {
			...
			if (
				(src || (srcset && srcset !== 'null')) &&
				elem.classNames.toLowerCase().indexOf('lazy') === -1
			) {
				return;
			}
			...
			for (const [name, value] of Object.entries(elem.attributes)) {
				...
				if (copyTo) {
					...
					if (elem.tagName === 'IMG' || elem.tagName === 'PICTURE') {
						...
						elem.tagName === 'FIGURE' &&
						!elem.querySelectorAll('img, picture').length
					) {
		...
		noscripts.forEach((noscript) => {
			...
			if (prevElement && isSingleImage(prevElement)) {
				...
				if (prevImg.tagName !== 'IMG') {
					prevImg = prevElement.getElementsByTagName('img')[0] as HTMLElement;
				}
