...
export function findMeta(metaEls: HTMLElement[], name: string) {
	const meta = metaEls.find(
		(el) =>
			el.getAttribute('name') === name || el.getAttribute('property') === name,
	);
...
/**
 * Gets the "expanded" URL for a given URL.
 */
 ...
 * @param Element
 **/
export function isSingleImage(node: HTMLElement): boolean {
	if (node.tagName === 'IMG') return true;
	if (node.childNodes.length !== 1 || node.textContent.trim() !== '')
		return false;
	return isSingleImage(node.childNodes[0] as HTMLElement);
}
const regexes = [
	/(blog|website)\s+of\s+(.*)?$/,
